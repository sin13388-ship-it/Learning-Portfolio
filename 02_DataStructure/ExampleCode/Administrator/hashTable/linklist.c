...
{
	strcpy(node->student.name,student_info->name) ;	
	node->student.age = student_info->age ;
}
...
Node * llist_search_student(LinkList * linklist, Student * student_info) 
{
 ...
 {
		if(strcmp(tempNodePtr->student.name,student_info->name) == 0 &&
		    tempNodePtr->student.age == student_info->age)
		{
			return tempNodePtr ;	
		}
