...
{	u8 i,Buffer[14];
	Buffer[0]=cmdASC; Buffer[1]=0; Buffer[2]=14;	//裝載命令
	...
	for(i=0;i<8;i++) Buffer[6+i]=(h&(0x80>>i))?'1':'0';
	e_DataRAM(Buffer,14);							//送出參數
...
{	u8 i=0,retval=0; 								//2021.12.31更新
	...
	SCL=0; 											//設定 SCL=0
	GCC_DELAY(65*FreqFactor); 						//for eDISPLAY Access Time
	return retval;
}
