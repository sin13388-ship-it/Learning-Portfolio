// PROGRAM	: HkDotMatrixV2.c						2019.0306
...
{	_wdtc=0b10101111;								//關閉看們狗計時器
	...
	_emi=1;											//致能EMI
	Copy2Buffer(TAB);								//複製字型
 	while(1)
	{	ptr=Buffer;									//Initial Pointer			
		_tb0on=1; Delayms(100);						//Satrt TB0 & Delay 100ms					
		while(ptr!=Buffer);
...
{	u8 i;
	...
	{	Buffer[i]=0;								//Clear Buffer[0]~Buffer[7]
		//Buffer[i+8]=*ptr++;						//Copy Pattern to Buffer[8]~Buffer[15]
		Buffer[15-i]=*ptr++;						//Copy Pattern to Buffer[8]~Buffer[15] with Reverse Order
	}
}
void ShiftUp(void)
{	u8 i,temp;	
