// PROGRAM	: U4_2_2V2.c (LCM 4-Bit Mode)		2025.0730
...
{	u16 adrs, cmd; u8 state,i;
	...
	LCMInit();
	LCMWrite4(0,0x80); i=0;							//第一列
	while(STR1[i]!=0) LCMWrite4(1,STR1[i++]);		//顯示STR1[]字串
	LCMWrite4(0,0xC0);i=0;							//第二列
	while(STR2[i]!=0) LCMWrite4(1,STR2[i++]);		//顯示STR2[]字串
	while(1)
	{	state=IRDecoder(&adrs,&cmd);				//讀取IR信號
		switch (state)
		{	case 0: 
				LCMWrite4(0,0xCF); LCMWrite4(1,'.');//非NEC格式
				break;
			case 1:
				LCMWrite4(0,0x8F); LCMWrite4(1,'R');//重複碼(Repeat Code)
				break;	
			case 2:
				LCMWrite4(0,0x89); Display(&adrs);	//第一列，顯示ID
				LCMWrite4(0,0xC9); Display(&cmd);	//第二列，顯示命令
				break;
		}	
		_st2on=1; Delayms(30); _st2on=0; Delayms(20);//輸出2KHz方波並維持30ms
		_st2on=1; Delayms(60); _st2on=0; Delayms(20);//輸出2KHz方波並維持60ms
		LCMWrite4(0,0x8F); LCMWrite4(1,' ');
		LCMWrite4(0,0xCF); LCMWrite4(1,' ');
	}
}
...
{	u32 temp=0,temp1=1;
	...
	{	_pt2on=1;									//啟動計數
		...
		if(TCnt>3375) temp|=temp1;					//若位元時間>1.6875ms,則位元為1 
		temp1<<=1; 
	}	 
	_pt2on=0; 										//停止PTM2 
...
{	pLCMEN=0; pLCMRW=0; pLCMRS=0;	        		//設定EN/RW/RS腳位為低電位
	...
	LCMWrite4(0,0x28);								//功能設定:4-Bit, 2-Line, 5*7
	LCMWrite4(0,0x0C);								//顯示控制:Display on, No Cursor
	LCMWrite4(0,0x06);								//進入模式:Increse, Not Shift
	LCMWrite4(0,0x01);								//清除LCM
}
u8 LCMWrite4(_Bool DCFlag, u8 CD)
{	u8 BusyAC;
	LCMDataC|=0b11110000;							//Config LCM_DataPort[7:4] as I/P
	pLCMRS=0; pLCMRW=1;				       			//RS=0,RW=1 (Read IR)
	GCC_NOP(); 
	do
	{	pLCMEN=1; GCC_NOP();						//EN=1
		BusyAC=LCMData;								//Read Data[7:4] From LCM
		pLCMEN=0;									//EN=0
		BusyAC &= 0b11110000;						//Mask Low Nibble
		pLCMEN=1; GCC_NOP();						//EN=1
		BusyAC |= (LCMData>>4);						//Read Data[3:0] from LCM
		pLCMEN=0;									//EN=0
	} while (BusyAC&0b10000000);					//Loop until BF=0
	LCMDataC &= 0b00001111;							//Config LCM_DataPort[7:4] as O/P Mode
	pLCMRW=0;										//RW=0 (Write)
	pLCMRS=DCFlag;									//Set RS State By DCFlag
	LCMData &= 0b00001111;							//Mask Port State[7:4]
	LCMData |= (CD&0b11110000);						//Write CD[7:4] to LCM		
	pLCMEN=1; GCC_NOP(); pLCMEN=0;					//EN=1
	LCMData &= 0b00001111;							//Mask Port State[7:4]
	LCMData |= (CD<<4);								//Write CD[3:0] to LCM		
	pLCMEN=1; GCC_NOP(); pLCMEN=0;					//EN=1
	return BusyAC;	
}
